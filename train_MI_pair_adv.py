...
from MI_SegNet import Mine_Conv, Recon_encoder_fusion, Recon_decoder_fusion, Seg_encoder_LM,Seg_decoder_LM,Recon_encoder_LM, Recon_decoder_LM
...
# Seg_encoder = Seg_encoder_N(input_channel,z_dim,64).to(device)
# optimizer_seg_en = optim.Adam(Seg_encoder.parameters(), lr=lr)

# Seg_decoder = Seg_decoder_N(input_channel,z_dim,64).to(device)
...
optimizer_rec_en = optim.Adam(Rec_encoder.parameters(), lr=lr)
# scheduler_rec_en = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_rec_en,mode='min',patience=10)
...
optimizer_rec_de = optim.Adam(Rec_decoder.parameters(), lr=lr)
# scheduler_rec_de = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_rec_de,mode='min',patience=10)

Seg_encoder = Seg_encoder_LM(input_channel,init_features=64,num_blocks=2).to(device)
optimizer_seg_en = optim.Adam(Seg_encoder.parameters(), lr=lr)
# scheduler_seg_en = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_seg_en,mode='min',patience=10)

Seg_decoder = Seg_decoder_LM(input_channel,init_features=64,num_blocks=2).to(device)
optimizer_seg_de = optim.Adam(Seg_decoder.parameters(), lr=lr)
# scheduler_seg_de = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_seg_de,mode='min',patience=10)
...
print('learning rate:',optimizer_seg_en.param_groups[0]['lr'])

transform_image=transforms.Normalize(0.5,0.5)


trainable_params_seg_en = sum(
	p.numel() for p in Seg_encoder.parameters() if p.requires_grad
)
trainable_params_seg_de = sum(
	p.numel() for p in Seg_decoder.parameters() if p.requires_grad
)
...
trainable_params_mine = sum(
 	p.numel() for p in Mine.parameters() if p.requires_grad
)

print('Seg encoder size:', trainable_params_seg_en, 'Seg decoder size:', trainable_params_seg_de, trainable_params_seg_en+trainable_params_seg_de)
print('Rec encoder size:', trainable_params_rec_en, 'Rec decoder size:', trainable_params_rec_de)
print('Mine size:', trainable_params_mine)
print('Total model size:', trainable_params_seg_en+trainable_params_seg_de+trainable_params_rec_en+trainable_params_rec_de+trainable_params_mine)
    ...
    for i, data in enumerate(trainloader):
        ...
        if (i + 1) % 100 == 0:
            ...
            with torch.no_grad():
                ...
                # scheduler_rec_en.step(test_avg_seg_loss)
                # scheduler_rec_de.step(test_avg_seg_loss)
                # scheduler_seg_en.step(test_avg_seg_loss)
                # scheduler_seg_de.step(test_avg_seg_loss)
                
                test_avg_mutual_loss /= (idx+1)
                train_avg_seg_loss /= 100
